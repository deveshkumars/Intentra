...
describe('config', () => {
  ...
  describe('getRemoteSlug', () => {
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:deveshkumars/Intentra.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('deveshkumars-Intentra');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/deveshkumars/Intentra.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('deveshkumars-Intentra');
    });
    ...
    test('parses HTTPS remote URLs without .git suffix', () => {
      const url = 'https://github.com/deveshkumars/Intentra';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('deveshkumars-Intentra');
    });
