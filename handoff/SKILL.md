...
### Step 1: Gather context from the human

Use AskUserQuestion with these questions.

**Question 1:** "What was the prompt — what were you (or the agent) asked to do?"
- Header: "Prompt"
- Options:
  - A) "Infer from recent commits" — read `git log --oneline -10` and propose a summary
  - B) "Let me type it"
...
<The original ask — what was the human intent, verbatim or near-verbatim.
This is the "why." Paste the actual prompt if available. If not, reconstruct
it faithfully from context.>
...
Every factual claim in the handoff (commit hashes, file paths, branch names,
test results) MUST be verified via git/bash before writing. Run the command,
read the output, then write the claim. Never guess.
