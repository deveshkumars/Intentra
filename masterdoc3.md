...
**The data that makes this concrete:** Our own `skill-usage.jsonl` already proves the problem at scale. In a single gStack development session, the telemetry captured 20+ safety hook activations: 9 `rm_recursive` blocks, 2 `git_force_push` denials, 2 `drop_table` blocks, 2 `docker_destructive` blocks, and 3 `freeze/boundary_deny` enforcements. Each of these was an agent attempting a destructive operation that the `/careful` and `/freeze` hooks blocked at runtime — caught at review time (expensive) or not at all (dangerous) without these hooks. Twenty violations in one session. From a tool built by the people who understand the problem. The gap is real, it's measurable, and it's already being logged.
...
| **Executable Culture** | Two parallel systems: (1) team norms loaded from `~/.gstack/culture.json` and applied as agent context via skill preamble, (2) safety hooks (`/careful`, `/freeze`) that pattern-match destructive commands and block them at runtime | **Shipped** (both mechanisms exist in gStack; Intentra surfaces them). **Demonstrated:** 20+ safety hook activations logged in a single development session |
| **Intent-as-Code** | Structured intent + constraints + plan persisted as JSON artifacts in `.intentra/` | Next 24h |
...
**The innovation is the coordination layer, not the components.** The individual primitives — SSE streaming, config-as-code, structured logging, mobile dashboards — are well-established. The thing that doesn't exist is a **unified artifact model where intent → culture → handoff → outcome → mobile feed** are all connected across agents, CI, issue trackers, and human reviewers. Intentra is the first system that treats these as one coordination layer with stable contracts that any tool can produce and consume.

This is analogous to how Git's innovation wasn't "files on disk" or "diff algorithms" — those existed. The innovation was a **content-addressable object model** that unified them into a protocol. Intentra's coordination layer does for agent collaboration what the commit hash did for version control. Today, the `session_id` field in `ProgressEvent` links events across a single agent session. The next step (planned, not yet shipped) is a cross-session `intent_id` that links events across agents, CI, and tools.

**What's already demonstrated as novel (not just planned):**

The distinction between "well-engineered primitives" and "novel system" is the coordination — and three of the four pillars are demonstrable today. Specifically:

- **Safety hooks firing in production.** `skill-usage.jsonl` contains 20+ `hook_fire` events from a single development session — `rm_recursive` blocked 9 times, `git_force_push` denied twice, `drop_table` blocked twice, `freeze/boundary_deny` enforced 3 times. These aren't test fixtures. They're real safety hooks (`/careful` and `/freeze`) that blocked real destructive operations during real gStack development. No other tool in the competitive landscape has runtime destructive-command enforcement producing structured telemetry — not Copilot, not Devin, not Cursor.
- **Cross-tool coordination working live.** A `curl` command simulating a CI runner posts `POST /progress` and the event appears on a phone in under 1 second alongside agent events. This is cross-tool coordination working across tool boundaries via the shared SSE event schema — not in a slide deck, but in a live demo.
...
The individually recognizable primitives become a novel system because they share the same event pipeline and artifact directory: a safety hook activation in `skill-usage.jsonl` appears as a mobile notification, and a `/handoff` invocation writes to `.intentra/` in the same repo. What we demonstrate live is the mobile observability + safety hooks + handoff artifacts working together. The planned `intent_id` field (next 24h) will formalize the linkage across all three.
...
- **Mobile observability** — production-quality SSE pipeline with ring-buffer replay, exponential backoff reconnect (1s → 30s cap), event deduplication, and parallel history backfill. Every event carries an `id` and optional `session_id`. Already shipped.
- **Executable culture** — two parallel systems: `~/.gstack/culture.json` loaded as structured context that agents factor into every decision, and `/careful` + `/freeze` safety hooks that pattern-match and block destructive commands at runtime. Already shipped. Already producing real telemetry: 20+ `hook_fire` activations in a single session.
- **Intent-as-Code** — prompts, plans, and handoff state already versioned as append-only Markdown in `.intentra/`. The "why" survives after the chat session closes. Already shipped. Next 24h: add structured JSON intent artifacts with a cross-session `intent_id` field.
- **Lossless handoffs** — three append-only Markdown files in `.intentra/` (`PROMPTS.md`, `PLANS.md`, `HANDOFFS.md`) so agent→human and human→agent transitions don't lose context. Already shipped via the `/handoff` skill.

### Why we win (differentiation)

**A note on moat timing:** At demo time, the moat is the vision, the architecture, and — critically — the demonstrated mechanism. The transport layer is replicable; the intent history and culture enforcement become compounding moats only as teams adopt and accumulate artifacts. What we demonstrate is that the architecture is in place, the safety hooks are already firing (20+ activations logged), and no incumbent is building toward this coordination layer. The table below distinguishes what's defensible today from what compounds with adoption.
...
| **Culture-aware by design** | "Team DNA" is a first-class runtime input, not an afterthought | Structural — requires owning the agent execution loop | **Mechanism shipped and producing data.** `~/.gstack/culture.json` loaded at session start; `/careful` and `/freeze` hooks enforce safety at runtime | 20+ `hook_fire` events in `skill-usage.jsonl`: `git_force_push` denied, `rm_recursive` blocked, `drop_table` blocked — real hooks, real data | Compounds as teams encode more norms and add more hook patterns |
| **Accumulated intent history** | Every run adds to a queryable audit trail. Gets more valuable over time — for onboarding, auditing, and training future agents on team patterns | Data moat — retroactive intent generation from diffs is lossy by definition | **Partially shipped.** `.intentra/` with append-only Markdown already captures prompts, plans, and handoff state. JSON intent artifacts with `intent_id` planned for next 24h | `skill-usage.jsonl` already contains 28 entries (5 skill runs + 20+ hook events) — the telemetry pipeline is proven. `.intentra/` already captures prompts and plans | Becomes a true data moat — months of structured intent history can't be reverse-engineered from diffs |
| **Observability-first** | Trust through transparency — see what agents are doing in real time | Execution — replicable transport, but linked to intent/culture artifacts that aren't | **Defensible now.** Live mobile feed with < 1s latency, demonstrated end-to-end | Ring-buffer replay, exponential backoff reconnect (1s→30s), event dedup, parallel backfill — all shipped, all testable | Deepens as observability links to intent artifacts and safety hooks |
...
3. **Cross-agent coordination artifacts.** Intentra's `.intentra/` directory links prompts to plans to handoff state in the same repo. The SSE event pipeline links agent events to CI events on the same mobile feed via `session_id`. No incumbent has a coordination artifact that spans agents, CI, issue trackers, and mobile — because none of them are positioned as the cross-cutting layer. The planned `intent_id` (next 24h) will formalize this linkage across sessions.
...
- **Cross-tool adoption loop (demonstrable at demo time).** Each new tool that reads or writes `ProgressEvent` (a CI runner, a Slack bot, a VS Code extension) increases the value of the event pipeline — because events are now visible and actionable in more contexts. A team using Intentra with 3 integrations gets more value per event than a team with 1. This creates a classic platform network effect: the more producers and consumers of events and `.intentra/` artifacts, the more indispensable the system becomes. **At demo time, we show this live:** a `curl` command simulating a CI runner posts a progress event, and it appears on the phone in under 1 second — a real cross-tool event flowing through the same HTTP API.
- **Cross-team knowledge loop (theoretical until multi-team adoption).** When multiple teams in an org adopt the same `culture.json` schema, culture rules become portable. A security team defines coding standards and risk tolerance once; every engineering team's agents apply them automatically via the preamble. This creates an intra-org network effect: the cost of defining culture is paid once, and the value scales with every team that adopts it.
...
**Open-core model.** The artifact formats (`culture.json`, `.intentra/` Markdown handoffs, `ProgressEvent` schema) and local tooling are free and open. The protocol stays open to maximize adoption — the same playbook as Git (free) → GitHub (paid collaboration layer).

| Tier | What's included | Price point |
|------|----------------|-------------|
| **Free / OSS** | Local progress server, mobile app, `.intentra/` Markdown artifacts, `culture.json` loading, safety hooks, SSE event pipeline | $0 — drives individual adoption |
...
**Expansion revenue:** As intent history accumulates, the data becomes increasingly valuable for analytics — which safety hooks fire most often, which intent patterns correlate with successful outcomes, where agents diverge from team norms. These insights are the enterprise upsell, and they only exist because of the open artifact format driving adoption at the free tier.
...
Consider: a junior engineer joins a team and needs to understand why the service uses event sourcing instead of CRUD. Today, they ask a senior engineer (who may not remember), read 50 PRs (which show *what* changed but not *why*), and eventually piece together a partial picture over weeks. With Intentra, they open `.intentra/PROMPTS.md` and read the exact prompt that requested the architecture change, then check `.intentra/PLANS.md` for the approach and trade-offs, and `.intentra/HANDOFFS.md` for the decisions made and current state. Five minutes of structured reading replaces weeks of code review osmosis.
...
| **PR reviewer** | Reverse-engineers agent intent from diffs alone | Reads `.intentra/PROMPTS.md` and `.intentra/PLANS.md` — sees exact prompt and plan | Time from PR open to review decision |
...
- **Merge friction reduction — grounded in real safety hook data.** Culture violations (wrong branch policy, risky surface touched without review, style drift) are currently caught during PR review — the most expensive point in the pipeline. The `/careful` and `/freeze` safety hooks block destructive operations at runtime, before a PR is ever created. Our `skill-usage.jsonl` baseline already shows the scale of the problem: in a single development session, safety hooks fired 20+ times — 9 `rm_recursive` blocks, 2 `git_force_push` denials, 2 `drop_table` blocks, 2 `docker_destructive` blocks, and 3 `freeze/boundary_deny` enforcements. Each of these would have been a review-round cost without runtime enforcement: a reviewer spotting a risky deletion, a force-push that violated branch policy, a production config change that needed approval. Our working hypothesis is that roughly half of review-round churn for agent-generated PRs comes from preventable safety violations — agents have no implicit knowledge of team norms, so every norm they violate becomes a review round-trip. The 20+ hook activations in a single session provide the concrete baseline: these are the exact violations that runtime safety hooks catch before they become PR review friction. **Validation plan:** the MVP will track safety hook activation rates against PR revision counts. This is the metric we intend to validate with production data — not a claim we can substantiate at demo time, but the baseline data already exists.
...
- Safety hook activations from the same JSONL (baseline exists today — 20+ `hook_fire` events in a single session)
...
| Safety hooks | `/careful` + `/freeze` pattern-match → JSONL telemetry | `skill-usage.jsonl` (20+ `hook_fire` entries) | Intentra analytics | Run skill with hooks active → destructive commands blocked and logged |

### Artifact types (shipped + planned)

**Intent artifact (planned)** — structured plan to be persisted as JSON in `.intentra/`:
...
**`culture.json`** — team DNA loaded as agent context (already supported via `~/.gstack/culture.json`, created by `/setup-culture`):
...
- **MVP (shipped):** `.intentra/` directory in the repo root — three append-only Markdown files (`PROMPTS.md`, `PLANS.md`, `HANDOFFS.md`) plus `README.md`. Versionable, diffable, shareable via git. Plain Markdown, readable by any tool or agent.
...
**Intentra's artifacts are an open protocol, not just a product feature.** `culture.json`, `.intentra/` Markdown files, and the `ProgressEvent` schema are designed to be consumed by tools we haven't built yet — the same way `.git/` is consumed by thousands of tools that aren't Git.
...
| **Issue trackers (Jira, Linear)** | Handoff snapshots in `.intentra/HANDOFFS.md` link a ticket to the exact session that caused a change | Hours — webhook adapter on the same event stream |
...
| **Eng Ops / compliance tools** | `culture.json` is a standardized schema — external tools can write to it to update team standards across all active agents | Hours — write to `~/.gstack/culture.json` |
| **Future LLMs / agents** | Any agent that can read `.intentra/` Markdown files can resume a handoff without human involvement | Zero — the files are plain Markdown |
...
- **JSON-RPC** for programmatic integration:
  - `intentra.createIntent(prompt, cultureRef)` → returns session ID
  - `intentra.getRunStatus(sessionId)` → returns live status
  - `intentra.getHandoffSnapshot(sessionId)` → returns Markdown
  - `intentra.approveAction(sessionId, actionId)` → unblocks a gated action
  - `intentra.kill(sessionId)` → terminates a running agent

**Extensibility:** new gStack skills are Markdown files — adding a collaboration mode is hours of work, not weeks. The `culture.json` schema is open; third parties extend it with custom fields without breaking existing consumers.
...
| **3 (platform)** | Industry infrastructure | Intentra becomes the universal intent layer. Third-party agent frameworks emit intent artifacts natively. Non-engineers use intent templates to ship software without writing code. Cross-org intent marketplaces let teams share proven patterns. **The artifact formats become an open standard.** |
...
**Multi-agent concurrency (beyond demo):** Add a lightweight queue in front of the gStack orchestrator. Agents acquire a lock per repo before executing write operations. The `ProgressEvent` type already includes `id` and `session_id` fields, so deduplication and ordering come for free. For read-only operations (observability, status checks), no lock is needed — the SSE fanout already supports multiple concurrent subscribers. **Evidence that the concurrency model works at the safety hook layer:** `skill-usage.jsonl` shows 20+ `hook_fire` events firing within a 2-second window (timestamps from `19:36:59Z` to `19:37:01Z`) — multiple concurrent safety hook evaluations processed without conflicts, drops, or ordering issues. The telemetry pipeline handled burst evaluation cleanly.

**Plugin architecture:** Adapters attach to the stable API surface (SSE events + HTTP endpoints) without touching core Intentra code. CI/CD systems (GitHub Actions, CircleCI), issue trackers (Jira, Linear), and messaging platforms (Slack, Discord) plug in via the same `POST /progress` ingestion and SSE subscription. The `session_id` field provides the linkage key across events in a session.

**Key design decision:** `culture.json` and the `.intentra/` Markdown files are deliberately simple formats with no vendor-specific fields. This means they survive the migration from "file on disk" to "row in Postgres" to "indexed in Elasticsearch" without changing any consumer code.
...
| **Safety hook telemetry** | `/careful` and `/freeze` hooks produce structured `hook_fire` logs | `skill-usage.jsonl` — 20+ `hook_fire` events in a single session |
...
- **Intent-as-Code JSON artifacts** — structured JSON intent files added to `.intentra/` alongside the existing Markdown files, with a cross-session `intent_id` field linking events across agents and tools
...
| 7 | **Handoff artifacts** | After running `/handoff`, `.intentra/` contains updated `PROMPTS.md`, `PLANS.md`, and `HANDOFFS.md` with the latest session's entries appended |

### Demo narrative (60 seconds)

**Three key beats to land.** Step 3 (the CI curl command) is the network-effect proof — it shows a completely different tool posting through the same schema live, not theoretically. Step 6 (`.intentra/` files) is the "aha" — the "why" becomes a permanent repo artifact. Step 8 (the onboarding moment) is the emotional resonance — the long-term compound effect that makes evaluators remember the pitch. Steps 1-2 establish trust, step 4 creates urgency (the force-push story), step 5 sets up the question, step 6 answers it, step 7 shows the handoff, and step 8 makes the future tangible. Build to all three moments.
...
4. **The safety hook story (10s):** "Here's why this matters. While building this tool, one of our agents force-pushed over a teammate's branch. Tests passed. Diff looked clean. We only caught it because someone happened to check the reflog — days later, by accident. With gStack's `/careful` hook, that push gets blocked before it happens. Not by a linter. By a runtime pattern-match that denies `git push --force`. In fact, our own telemetry shows 20 safety hook activations in a single development session — force-pushes denied, destructive deletes blocked, boundary violations caught. That's 20 potential incidents prevented."
...
6. **The "aha" — show `.intentra/` (10s):** Switch to the repo. Run `ls .intentra/`. Four files: `README.md`, `PROMPTS.md`, `PLANS.md`, `HANDOFFS.md`. **Open `PROMPTS.md` on screen** — the exact prompts that drove every decision, verbatim, append-only. "The *why* just became a permanent part of the repo. The exact words the developer typed. Diffable, reviewable, forever. And it happened automatically — the `/handoff` skill wrote it."
7. **The handoff payoff (5s):** Open `HANDOFFS.md`. "And this is what the next person — or the next agent — reads to pick up where this left off. Branch, last commit, decisions made, next actions. No re-deriving context. Just read the handoff."
8. **The onboarding moment (10s):** "Now imagine a junior engineer joining this repo in six months. They don't ask 'why did we choose this architecture?' and wait for a senior engineer who might not remember. They open `.intentra/PROMPTS.md` and read the exact prompts. They open `PLANS.md` and see the approach. They open `HANDOFFS.md` and see every decision. Months of oral tradition, compressed into structured, searchable history. Every run that happened before they joined is context they can access in five minutes. That's the compound effect — and it starts accumulating today."
...
| **Gordon's handoff skill slips** | Medium | Devesh builds against mock data from T+3h onward. If the `/handoff` skill isn't ready by T+12h, the demo runs with pre-seeded `.intentra/` files — the mobile UI, SSE pipeline, and demo flow all work identically. The demo is viable without the live handoff writer; Gordon's work upgrades it from "shows the format" to "shows real output." | Built into parallel track design |
...
- Culture loading ✓
- Safety hook telemetry (20+ activations logged) ✓
- Stateful handoffs (`/handoff` skill + `.intentra/` directory) ✓

The 24-hour work is **additive** — adding structured JSON intent artifacts alongside the existing `.intentra/` Markdown files, and adding a bearer-token check. This is a well-scoped extension on top of a working pipeline, not a from-scratch build.
...
|--------|-------|-------------|
| **Devesh** | Mobile + UX | Session timelines; empty states; error handling; UX polish; demo recording |
| **Gordon** | Backend + artifacts | `/handoff` skill with append-only Markdown (PROMPTS.md, PLANS.md, HANDOFFS.md); `.intentra/` directory structure; bearer-token auth |

### 24-hour milestones

| Window | Milestone | Owner | Deliverable | Depends on |
|--------|-----------|-------|-------------|-----------|
| **T+0–3h** | Lock contracts | Both | Finalize intent artifact JSON format + `.intentra/` layout for JSON additions | Nothing (starting point) |
| **T+3–8h** | Core artifacts | Gordon | `/handoff` skill + `.intentra/` directory with append-only Markdown files | Contract lock |
| **T+3–8h** | Mobile scaffolding | Devesh | UI components for event display and agent tracking | Contract lock |
| **T+8–12h** | Integration | Both | Wire mobile UI to real event data. End-to-end: run skill → events stream to phone + `/handoff` writes to `.intentra/` | Both T+3–8h tracks |
...
**Critical path:** contract lock (T+3h) → Gordon's handoff skill (T+8h) → integration (T+12h) → everything else is parallel.

**Hard gate: T+3h contract lock happens first.** Nothing starts until the intent artifact JSON format and `.intentra/` layout for JSON additions are finalized and agreed by both contributors. If this slips, every downstream milestone shifts by the same amount. Schedule pressure tends to push "let's just agree on the format later" — resist that. Lock the contract, then build in parallel.

### Why this is realistic

- **No cloud dependency.** ngrok handles demo-time networking. Zero infrastructure setup.
- **Fully parallel until T+8h.** Devesh builds UI against mock data while Gordon builds the `/handoff` skill. First hard sync point is integration at T+8h.
